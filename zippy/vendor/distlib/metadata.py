...
class Metadata(object):
    ...
    #FIXME: UPSTREAM
    pep459_keys = {
        'commands': 'extensions python.commands',
        'exports': 'extensions python.exports exports',
        }

    def __getattribute__(self, key):
        common = object.__getattribute__(self, 'common_keys')
        mapped = object.__getattribute__(self, 'mapped_keys')
        pep459 = object.__getattribute__(self, 'pep459_keys')
        #FIXME: UPSTREAM
        remainder = pep459.get(key, key).split()
        remainder.reverse()
        key = remainder.pop()
        if key in mapped:
            lk, maker = mapped[key]
        ...
        elif not remainder and key not in common:
        ...
        #FIXME: UPSTREAM
        while remainder:
            result = result.setdefault(remainder.pop(), dict())
        return result
    ...
    def _to_legacy(self):
        ...
        #FIXME: UPSTREAM
        for nks, ok in self.LEGACY_MAPPING.items():
            nmd = self._data
            try:
                for nk in nks.split():
                    nmd = nmd[nk]
